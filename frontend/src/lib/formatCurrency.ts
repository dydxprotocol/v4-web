...
export function formatCurrency(
  ...
  if (compact) {
    return formatCompactCurrency(numValue, { showSymbol, symbol, decimals });
  }
...
function formatCompactCurrency(
  value: number,
  options: { showSymbol: boolean; symbol: string; decimals: number }
): string {
  const { showSymbol, symbol, decimals } = options;
  ...
  const formatted = compactValue.toFixed(compactDecimals);
