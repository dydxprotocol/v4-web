...
import { expandDecimals, USER_0_ADDRESS, USER_1_ADDRESS, LIQUIDATOR_ADDRESS } from "./utils"
import pg from "pg"
import { BTC_ASSET, ETH_ASSET } from "./utils"
...
describe("Verify Liquidation", () => {
    ...
    it("should have liquidation events marked as latest for closed positions", async () => {
        const liquidations = await client.query(
            'SELECT latest FROM position WHERE change = $1',
            ['LIQUIDATE']
        )
        expect(liquidations.rows.length).toBe(3)
