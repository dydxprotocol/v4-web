...
import { call, getArgs, moveBlockchainTime, toPrice, walletToAddressIdentity, expandDecimals, DEFAULT_SUB_ID, USDC_ASSET, BTC_ASSET, ETH_ASSET, BTC_MAX_LEVERAGE, getUsdcConfig, getBtcConfig, getEthConfig, ETH_MAX_LEVERAGE } from "./utils"
...
async function populateEvents(taskArgs: any) {
    ...
    await call(vaultLiquidator.functions.liquidate_position(user0Identity, BTC_ASSET, true, liquidatorIdentity))
    await moveBlockchainTime(provider, 5, 1)
    ...
    await call(vaultLiquidator.functions.liquidate_position(user0Identity, ETH_ASSET, true, liquidatorIdentity))
