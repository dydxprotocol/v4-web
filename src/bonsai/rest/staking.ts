...
export const useStakingDelegations = () => {
  ...
  return useQuery({
    queryKey: ['delegations', address, client.key],
...
export const useUnbondingDelegations = () => {
  ...
  return useQuery({
    queryKey: ['unbonding-delegations', address, client.key],
    enabled: isPresent(address) && isPresent(client),
    queryFn: async (): Promise<UnbondingDelegation[]> => {
      ...
      const response =
        await client.compositeClient.validatorClient.get.getDelegatorUnbondingDelegations(address);
      const parsedResponse = parseToPrimitives(response);
...
export const useStakingRewards = () => {
  ...
  return useQuery({
    queryKey: ['staking-rewards', address, client.key],
    enabled: isPresent(address) && isPresent(client),
