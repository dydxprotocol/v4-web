...
export const getMarketOrders = createAppSelector(
  ...
  (orders): { [marketId: string]: SubaccountOrder[] | undefined } => {
    return (orders ?? []).reduce(
      ...
      {} as { [marketId: string]: SubaccountOrder[] | undefined }
    );
  }
);
...
export const getMarketSubaccountOpenOrders = createAppSelector(
  ...
  (
    ...
    [marketId: string]: SubaccountOrder[] | undefined;
  } => {
    return (orders ?? []).reduce(
      (marketOrders, order) => {
        marketOrders[order.marketId] ??= [];
...
export const getMarketFills = createAppSelector(
  [getSubaccountFills],
  (fills): { [marketId: string]: SubaccountFill[] | undefined } => {
    return (fills ?? []).reduce(
      (marketFills, fill) => {
      ...
      {} as { [marketId: string]: SubaccountFill[] | undefined }
    );
  }
);
...
export const getMarketFundingPayments = createAppSelector(
  ...
  (fundingPayments): { [marketId: string]: SubaccountFundingPayment[] | undefined } => {
    return (fundingPayments ?? []).reduce(
      ...
      {} as { [marketId: string]: SubaccountFundingPayment[] | undefined }
    );
  }
);
