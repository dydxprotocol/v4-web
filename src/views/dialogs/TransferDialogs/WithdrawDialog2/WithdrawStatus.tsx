...
import { selectSpotWithdraw, selectWithdraw } from '@/state/transfersSelectors';
...
export const WithdrawStatus = ({ id = '', type = 'perps', onClose }: WithdrawStatusProps) => {
  ...
  const perpWithdraw = useAppSelectorWithArgs(selectWithdraw, id);
  const spotWithdraw = useAppSelectorWithArgs(selectSpotWithdraw, id);

  const withdraw = type === 'spot' ? spotWithdraw : perpWithdraw;

  const transferSuccess = withdraw?.status === 'success';
  const transferError = withdraw?.status === 'error';
  const transferAssetRelease =
    type === 'perps' && perpWithdraw ? perpWithdraw.transferAssetRelease : undefined;
  ...
  const statusDescription = useMemo(() => {
    ...
    if (transferError) {
      ...
      if (type === 'spot' && spotWithdraw?.error) {
        return spotWithdraw.error;
      }
  ...
  }, [stringGetter, transferAssetRelease, transferError, transferSuccess, type, spotWithdraw]);

  const withdrawalOutput =
    withdraw == null ? (
      <Output tw="inline" value={null} type={OutputType.Fiat} isLoading />
    ) : type === 'spot' && spotWithdraw ? (
      <Output
        tw="inline"
        value={spotWithdraw.amount}
      ...
      <Output
        tw="inline"
        value={perpWithdraw.finalAmountUsd ?? perpWithdraw.estimatedAmountUsd}
        type={OutputType.Fiat}
        slotLeft={perpWithdraw.finalAmountUsd ? undefined : '~'}
        slotRight=" USDC"
      />
    ) : null;

  const withdrawalExplorerLinks =
    type === 'spot' && spotWithdraw?.txSignature
      ? [
          <Link
            key={spotWithdraw.txSignature}
            href={`https://solscan.io/tx/${spotWithdraw.txSignature}`}
            withIcon
            isAccent
            isInline
          >
            {truncateAddress(spotWithdraw.txSignature, '')}
          </Link>,
        ]
      : type === 'perps' && perpWithdraw
        ? perpWithdraw.transactions
            .map((t) => {
  ...
  return (
    <div tw="flex flex-col gap-1 px-2 pb-1.5 pt-2.5">
      ...
      <div tw="mt-auto flex items-center justify-between self-stretch">
        ...
        <div tw="flex items-center gap-0.125">
          ...
          {type === 'spot' ? (
            ...
            perpWithdraw && <AssetIcon symbol="USDC" chainId={perpWithdraw.destinationChainId} />
          )}
        </div>
      </div>
      <Button
      ...
      {withdrawalExplorerLinks.length > 0 && (
