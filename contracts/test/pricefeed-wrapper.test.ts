...
describe("PricefeedWrapper", () => {
    ...
    describe("mock", () => {
        it("should return correct positive price", async () => {
            ...
            const calculatedBtcPrice = underlying.upper.mul(new BN("18446744073709551616")).add(underlying.lower).sub(new BN("170141183460469231731687303715884105728"));
        ...
        it("should return correct negative price", async () => {
            ...
            // 2 ** 127 = 170141183460469231731687303715884105728
            // 2 ** 127 is zero for i128
            const calculatedBtcPrice = (new BN("170141183460469231731687303715884105728")).sub(underlying.upper.mul(new BN("18446744073709551616")).add(underlying.lower));
            expect(calculatedBtcPrice.toString()).to.equal(btcPrice.toString())
        })
    })
