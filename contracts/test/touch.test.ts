...
import { FungibleFactory, PricefeedWrapperFactory, StorkMockFactory, TestnetTokenFactory, VaultFactory } from '../types';
import { Fungible, Vault, StorkMock, PricefeedWrapper } from "../types"
...
describe("Vault.touch", () => {
    ...
    beforeEach(async () => {
        ...
        */
        ...
        const { contract: _vault } = await waitForResultVault()
        vault = _vault
    ...
    it("add liquidity", async () => {
        ...
        await call(
            vault_user0
                .functions.add_liquidity(user0Identity)
    ...
    it("remove liquidity", async () => {
        ...
        await call(
            vault_user0
                .functions.remove_liquidity(user0Identity)
                .addContracts(attachedContracts)
                .callParams({
    ...
    it("increase long position", async () => {
        ...
        await call(
            vault_user1
                .functions.increase_position(user1Identity, BTC_ASSET, expandDecimals(100), true)
                .addContracts(attachedContracts)
                .callParams({
                    forward: [expandDecimals(100), USDC_ASSET_ID],
                }),
    ...
    it("increase short position", async () => {
        ...
        await call(
            vault_user1
                .functions.increase_position(user1Identity, BTC_ASSET, expandDecimals(100), false)
                .addContracts(attachedContracts)
                .callParams({
                    forward: [expandDecimals(100), USDC_ASSET_ID],
                }),
    ...
    it("decrease long position", async () => {
        ...
        await call(
            vault_user1
                .functions.decrease_position(user1Identity, BTC_ASSET, position.collateral.div(2), position.size.div(2), true, user1Identity)
                .addContracts(attachedContracts),
        )
    ...
    it("decrease short position", async () => {
        ...
        await call(
            vault_user1
                .functions.decrease_position(user1Identity, BTC_ASSET, position.collateral.div(2), position.size.div(2), false, user1Identity)
                .addContracts(attachedContracts),
        )
        const positionAfter = (await vault.functions.get_position_by_key(positionKey).get()).value
        expect(positionAfter.size.gt(position.size.div(2).mul(95).div(100))).eq(true)
    ...
    it("profit long position", async () => {
        ...
        await call(
            vault_user1
                .functions.increase_position(user1Identity, BTC_ASSET, expandDecimals(1000), true)
                .addContracts(attachedContracts)
                .callParams({
    ...
    it("loss short position", async () => {
        ...
        await call(
            vault_user1
                .functions.increase_position(user1Identity, BTC_ASSET, expandDecimals(1000), false)
                .addContracts(attachedContracts)
                .callParams({
                    forward: [expandDecimals(400), USDC_ASSET_ID],
                }),
    ...
    it("liquidate long position", async () => {
        ...
        await call(
            vault_liquidator
                .functions.liquidate_position(user1Identity, BTC_ASSET, true, liquidatorIdentity)
                .addContracts(attachedContracts),
        )
        const positionAfter = (await vault.functions.get_position_by_key(positionKey).get()).value
    ...
    it("liquidate short position", async () => {
        ...
        await call(
            vault_liquidator
                .functions.liquidate_position(user1Identity, BTC_ASSET, false, liquidatorIdentity)
                .addContracts(attachedContracts),
        )
        const positionAfter = (await vault.functions.get_position_by_key(positionKey).get()).value
        expect(positionAfter.size.isZero()).eq(true)
